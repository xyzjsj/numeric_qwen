        ...
        if text is not None:
            ...
            for txt in text_list:
                ...
                for i, token_id in enumerate(input_ids):
                    if token_id == self.num_token_id:  # 记录<num>的位置，这是数值预测的触发点
                        positions.append(i)
...
class NumericQwen2_5_VLForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs
        ...
        if inputs_embeds is not None:
            ...
            base_inputs_embeds = self.get_input_embeddings()(input_ids).clone()
        else:
                    ...
                    if batch_idx < len(numeric_values) and numeric_values[batch_idx]:
                        ...
                        for value, pos in zip(values, positions):
                            ...
                            if 0 <= pos < base_inputs_embeds.shape[1]:
                                # pos现在是<num>的位置，直接在这个位置注入数值embedding
                                try:
                                    ...
                                    # 保留原始<num>embedding，与数值embedding相加
                                    original_emb = base_inputs_embeds[batch_idx, pos].clone()
                                    base_inputs_embeds[batch_idx, pos] = original_emb + numeric_emb
    ...
    def _compute_mixed_loss(
        ...
        # 不对predicted_floats做shift，保持原位置用于数值损失计算
        ...
        # 为数值损失创建mask，基于<num>位置（数值预测触发点）
        if num_token_id is not None and num_token_id >= 0:
            float_target_mask = (input_ids == num_token_id)
        else:
            float_target_mask = torch.zeros_like(input_ids, dtype=torch.bool)
            
        # token损失不需要特殊mask，所有位置都参与训练
        
        # 计算token损失（交叉熵）
        loss_fct_token = CrossEntropyLoss()
        token_labels = shift_labels.clone()
        # 不需要将num_pad后面的token设为-100，因为它们是正常的文本token，应该参与训练
        ...
        if numeric_values is not None and numeric_positions is not None:
            ...
            float_labels = torch.zeros_like(predicted_floats)
            ...
            if float_target_mask.any():
                # 直接使用基于原始input_ids的mask，形状完全匹配
                float_target_mask = float_target_mask.to(predicted_floats.device)
                float_labels = float_labels.to(predicted_floats.device)
                
                valid_float_preds = predicted_floats[float_target_mask]
                valid_float_labels = float_labels[float_target_mask]
